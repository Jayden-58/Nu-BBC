...
WHERE discontinued IS false AND units_in_stock <= reorder_level;
...
SELECT ship_address, COUNT(orders) AS order_count
FROM orders
...
FROM orders
GROUP BY customer_id HAVING SUM(freight) >= 500
...
FROM territories t
FULL JOIN regions r ON r.region_id = t.region_id
